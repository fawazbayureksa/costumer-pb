...
//referensi dari ProductDiscussion
class SpeakerDiscussion extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            page_count: 0,
    ...
    getData = () => {
        let url = `${process.env.REACT_APP_BASE_API_URL}webinar/discussion/getWithParams`
        let params = {
            page: this.state.page,
            per_page: 10,
            webinar_speaker_id: this.props.speakerDetail.id
        }
    ...
    reply = (id) => {
        ...
        if (IsEmpty(this.state["reply_" + id])) {
            ...
            let url = `${process.env.REACT_APP_BASE_API_URL}webinar/discussion/reply`
    ...
    send = () => {
        ...
        if (IsEmpty(this.state.new_discussion)) {
            ...
            let url = `${process.env.REACT_APP_BASE_API_URL}webinar/discussion/new_discussion`
            axios.post(url, data, Config({
                Authorization: 'Bearer ' + Cookie.get('token'),
            })).then(res => {
                SwalToast.fire({ icon: "success", title: res.data.message })
                ...
                {this.state.records.length > 0 ? <><div className="shadow-graph bg-white rounded">
                    {this.state.records.map((item, index) => (<div key={item.id} className="mb-4 p-3">
                        <div className={`${index < this.state.records.length - 1 && "border-bottom"}`}>
                            ...
                            {item.webinar_speaker_discussion_replies.length > 0 && item.webinar_speaker_discussion_replies.length > parseInt(this.state.showReplies[index] ? this.state.showReplies[index] : 1) &&
                            ...
                            {item.webinar_speaker_discussion_replies.map((reply, replyIndex) => (
                                replyIndex > item.webinar_speaker_discussion_replies.length - 1 - parseInt(this.state.showReplies[index] ? this.state.showReplies[index] : 1) &&
                                <div className="p-3 d-flex">
...
/**
 * @param {object} speakerDetail product detail
 */
export default withTranslation()(SpeakerDiscussion)