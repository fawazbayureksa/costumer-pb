...
class SpeakerListReview extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            page_count: 0,
    ...
    componentDidMount = () => {
        this.getData()
        this.getRating()
        console.log(this.props.speakerDetail.id)
    }
    ...
    getData = () => {
        let url = `${process.env.REACT_APP_BASE_API_URL}webinar/speaker/getSpeakerRatingWithParams`
        let params = {
    ...
    getRating = () => {
        let data = []
        let url = `${process.env.REACT_APP_BASE_API_URL}webinar/speaker/getRating`
        let params = {
            webinar_speaker_id: this.props.speakerDetail.id
        }
        axios.get(url, Config({
                ...
                {this.props.type == "type_2" && <div>
                    ...
                    <div className="bg-white shadow-graph rounded p-4">
                        <p className="font-weight-semi-bold">
                            {/* <ManualSwitchLanguage data={this.props.speakerDetail.mp_product_informations} langAttr={"language_code"} valueAttr={"name"} /> */}
                        </p>
                        {this.state.rating &&
                            <div className="d-flex mt-2">
                                <div className="d-flex align-items-center">
                                    <div>
                                        <p className="d-flex justify-content-center font-weight-semi-bold"><label className="">{this.props.speakerDetail.rating ? this.props.speakerDetail.rating : "-"}</label><label className=" d-flex align-items-end">/5</label></p>
 ...
 */
export default withTranslation()(SpeakerListReview)