...
const Cart = (props) => {
    ...
    const imgFit = () => {
        let size = $('.cart-widget-text').css('font-size');
        $('.cart-widget-text img').css({'width': size, 'height': size});
    }
    ...
    return (
        <>
            ...
            `}</style>
            <Link to={EcommerceRoutePath.CART} className="text-decoration-none position-relative d-flex align-items-center cart-widget-text" style={styles}>
                {props.data.logo &&props.data.logo_position === 'left' &&
                <CustomImage folder={PublicStorageFolderPath.cms} filename={props.data.logo} alt={props.data.logo} className="object-fit-cover mr-1" style={{width: 25, height: 25}}/>}
                {props.data.text}
                {props.data.logo && props.data.logo_position === 'right' &&
                <CustomImage folder={PublicStorageFolderPath.cms} filename={props.data.logo} alt={props.data.logo} className="object-fit-cover ml-1" style={{width: 25, height: 25}}/>}
